...
def main(_):
    ...
    if not os.path.isfile(prevstatepath) or not os.path.isfile(prevmodelpath):
        ...
        if not os.path.exists(modelpath + '.tar'):
            logging.info("Start compression with wf={}".format(wf))
    ...
    #                                             vbmf_weaken_factor= wf)
            ...
            logging.info('End compression with wf={}, .pth model has been saved {}'.format(wf, modelpath))


            end = time.time()
            logging.info("Time of compression: {}".format(end - start))  
